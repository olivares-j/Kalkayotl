...
file_plot  = dir_ana   + "Plots/1D_vs_all.pdf"
...
data.rename(index=str,inplace=True, 
			columns={"b_rest": "CBJ_min","rest": "CBJ_ctr","B_rest": "CBJ_max"})
...
for j,prior in enumerate(list_of_priors):
	#----- Name X ----------------------
	name_x = prior["type"]+"_1D"

	for i,case in enumerate(["3D","5D"]):
		#-------------- names -----------------
		name_y = prior["type"]+"_"+case
		...
		#--------- Uncertainty bars -------------------------------------------------------------------------
		x_err  = np.vstack((data[name_x+"_ctr"]-data[name_x+"_min"],data[name_x+"_max"]-data[name_x+"_ctr"]))
		y_err  = np.vstack((data[name_y+"_ctr"]-data[name_y+"_min"],data[name_y+"_max"]-data[name_y+"_ctr"]))

		#---------- Scatter plot ---------------------------
		plt.scatter(data[name_x+"_ctr"],data[name_y+"_ctr"],s=20,color=colors[j],marker=markers[i],zorder=i+1,label=label)

		#----------- Plot uncertainties ---------------------------------------------------------------------------
		# plt.errorbar(data[name_x+"_ctr"],data[name_y+"_ctr"],yerr=y_err,xerr=x_err,
		# 	label=prior["type"],#fmt='none',
		# 	ls='none',marker="o",ms=5,elinewidth=0.01,zorder=1)

	#----------- CBJ distances ------------------------------------
	plt.scatter(data[name_x+"_ctr"],data["CBJ_ctr"],s=20,color=colors[j],marker=markers[2],zorder=i+1,label=None)
		
plt.xlabel("Distance [pc]")
plt.ylabel("Distance [pc]")
plt.xlim(np.min(data["CBJ_ctr"])-10,np.max(data["CBJ_ctr"])+10)
plt.ylim(np.min(data["CBJ_ctr"])-10,np.max(data["CBJ_ctr"])+10)
plt.legend(
	title="Prior",
	shadow = False,
