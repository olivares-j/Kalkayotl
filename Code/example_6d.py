...
from posterior_6d import Posterior as Posterior_6d
...
#======================= 6D Version =====================================================
list_observables = ["ID_member","ra","dec","parallax","pmra","pmdec","radial_velocity",
...
for prior in list_of_prior:
    name_chains = "Chains_6D_"+str(prior["type"])+"_loc="+str(int(prior["location"]))+"_scl="+str(int(prior["scale"]))+".h5"
    file_chains = dir_chains+name_chains
    file_csv    = file_chains.replace("h5","csv")

    if not os.path.isfile(file_chains):
        p1d = Inference(posterior=Posterior_6d,
                        prior=prior["type"],
    ...
    #----------------- Analysis ---------------
    a1d = Analysis(n_dim=6,file_name=file_chains,id_name=list_observables[0],
        dir_plots=dir_plots,
