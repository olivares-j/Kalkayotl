...
file_out     = dir_graphs + "out_"+str(prior)+".csv"        # file where the statistics will be written.

###################### Initialise the parallax2distance class ################################

p2g = pps(pax,u_pax,nwalkers=50,distance_prior=prior)

theta = np.array([0,100.0])
a= p2g.lnprob(theta)
print(a)
###############################################################################################
sys.exit()
