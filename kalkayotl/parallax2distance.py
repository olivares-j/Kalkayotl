...
import tqdm
from posterior_distance import Posterior

class parallax2distance(object):
	"""
	...
	def infer(self,n_iter,n_walkers,file_chains="chains.h5",progress=True):
		"""
		Performs the MCMC run
		"""
		pos0 = self.frozen_uniform.rvs(size=(n_walkers,1))
		...
		for ID,datum in data.iterrows():
			...
			sampler = emcee.EnsembleSampler(n_walkers,1, self.Posterior, args=datum,
