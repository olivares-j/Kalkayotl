...
iocs      = [
			{"name":"-", "value":"minus", "linestyle":":"},
			{"name":"0", "value":"corr",  "linestyle":"-"},
			{"name":"+", "value":"plus",  "linestyle":"--"},
			]
...
if prior is "Gaussian":
	parameters = [
				{"name":"Location","xlim":[1000,5000],"ylim":[[-0.11,0.11],[0.0005,0.15],[0.0,101]],"idx_dp":[0,1,2]},
				{"name":"Scale",   "xlim":[90,2100],  "ylim":[[-0.15,0.75],[0.01,0.25],  [0.0,101]],"idx_dp":[0]}
				]
#------------------------------------------------------------------------------------------------
...
stats     = np.zeros((5,len(iocs),len(list_of_sources),len(distances),len(random_states)))
pars      = np.zeros((3,len(iocs),len(parameters),len(list_of_sources),len(distances),len(random_states)))
...
for p,par in enumerate(parameters):
	...
	dsts = [distances[i] for i in par["idx_dp"]]
	axes[2,p].set_xticks(dsts)
	...
	for i,ioc in enumerate(iocs):
		for n,scs in enumerate(list_of_sources): 
			...
			#--------------- Accuracy -------------------------------------------------------------------
			axes[0,p].fill_between(dsts,y1=mu_ferror-sd_ferror,y2=mu_ferror+sd_ferror,
								color=scs["color"],linestyle=ioc["linestyle"],alpha=0.1,label=None)
			axes[0,p].plot(dsts,mu_ferror,color=scs["color"],alpha=0.8,linestyle=ioc["linestyle"],label=None)

			#---------------- Precision -----------------------------------------------------------------
			axes[1,p].fill_between(dsts,y1=mu_span-sd_span,y2=mu_span+sd_span,
								color=scs["color"],linestyle=ioc["linestyle"],alpha=0.1,label=None)
